namespace TurnTheGameOn.SimpleTrafficSystem
{
    ...
    public class AITrafficWaypointRoute : MonoBehaviour
    {
        ...
        public void SpawnTrafficVehicles()
        {
            ...
            {
                //spawns a random number of cars. Specifications aobut minimum and maximum number of cars that can spawn is done in the unity editor. 
                if (spawnTrafficVehicles.Length > 0)
                {
                    int randomSpawn = UnityEngine.Random.Range(minSpawn, maxSpawn);
                for (int i = 0, j = 0; i < randomSpawn && j < waypointDataList.Count - 1; i++, j++)
                {
                    Vector3 spawnPosition = waypointDataList[j]._transform.position;
                    spawnPosition.y += 1;
                    GameObject spawnedTrafficVehicle = Instantiate(spawnTrafficVehicles[0], spawnPosition, waypointDataList[j]._transform.rotation);
                    spawnedTrafficVehicle.GetComponent<AITrafficCar>().RegisterCar(this);
